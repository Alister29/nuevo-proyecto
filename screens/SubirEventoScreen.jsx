...
import { db, storage } from "../database/firebaseConfig"; // Importar Firebase Storage
  ...
  if (!user) {
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>Debe iniciar sesión para subir documentos.</Text>
        <TouchableOpacity
          style={styles.botonIniciarSesion}
          onPress={() => alert('Por favor, inicie sesión')}
        >
  ...
  const subirDatos = async () => {
    ...
    try {
      ...
      for (const file of documentos) {
        ...
        archivosBase64.push({
          nombre: file.name,
          peso: `${(file.size / 1024).toFixed(0)}kb`,
          uri: base64ConPrefix,
        });
      }
